 ...
 *
 * Google Sheet can 3 tab:
 *   DN_LSX    : DN | Khach hang | LSX | So luong
 *   DN_ScanLog: SessionID | DN | LSX | Noi dung QR | So luong | Thoi gian | Nguoi thuc hien
 *   DN_KetQua : DN | LSX | SL yeu cau | SL thuc te | Trang thai | Thoi gian | Nguoi thuc hien
 *
...
var LOG_HEADERS = ["SessionID", "DN", "LSX", "Noi dung QR", "So luong", "Thoi gian", "Nguoi thuc hien"];
var KQ_HEADERS  = ["DN", "LSX", "SL yeu cau", "SL thuc te", "Trang thai", "Thoi gian", "Nguoi thuc hien"];
...
function doGet(e) {
  try {
    ...
    var ss = SpreadsheetApp.getActiveSpreadsheet();
    if (action === "version") return json_({ result: "ok", version: "2.0" });
...
function submitResult_(body) {
  ...
  if (summary.length > 0) {
    ...
    for (var j = 0; j < summary.length; j++) {
      ...
      kqOut.push([dnCode, String(s.lsx || ""), slYC, slTT, trangThai, timeStr, userName]);
    }
